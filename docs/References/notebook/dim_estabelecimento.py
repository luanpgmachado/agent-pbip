...
PIPELINE_NAME = 'dim_estabelecimento'
...
def pipeline(spark: SparkSession):
    ...
    pessoa_DF = \
        get_DF_from_delta_file(spark, os.path.join(LAKE_HOME, "stage_extract_pessoa_juridica")) \
            .select( \
                col("CD_CGC").alias("L_CD_CGC"), \
                col("DS_RAZAO_SOCIAL"), \
                col("NM_FANTASIA")
            )

    estabelecimento_DF = \
        get_DF_from_delta_file(spark, os.path.join(LAKE_HOME, "stage_extract_estabelecimento")) \
            .select( \
                col("RecordKey"), \
                col("CD_ESTABELECIMENTO"), \
                col("CD_CGC") \
            )

    df = \
        estabelecimento_DF.join(
            pessoa_DF,
            on=[(pessoa_DF.L_CD_CGC == estabelecimento_DF.CD_CGC)],
            how='left'
        ).select(
            col("RecordKey").alias("_id"),
            col("CD_ESTABELECIMENTO").alias("Estabelecimento___Codigo"),
            col("CD_CGC").alias("Estabelecimento___CNPJ"),
            col("DS_RAZAO_SOCIAL").alias("Estabelecimento___Nome_Razao_Social"),
            col("NM_FANTASIA").alias("Estabelecimento___Nome_Fantasia")
        )

    df = \
        create_surrogate_key_index(spark, df, table_name=PIPELINE_NAME, key_column_name="_id", column_name="Estabelecimento___Key")
